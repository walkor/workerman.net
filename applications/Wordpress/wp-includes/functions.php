...
function status_header( $header ) {
	...
	return App\Common\Protocols\Http\header($status_header, true, $header );
}
...
function nocache_headers() {
	...
	if ( function_exists( 'App\Common\Protocols\Http\header_remove' ) ) {
		App\Common\Protocols\Http\header_remove( 'Last-Modified' );
	} else {
	...
	foreach( $headers as $name => $field_value )
		App\Common\Protocols\Http\header("{$name}: {$field_value}");
}
...
App\Common\Protocols\Http\header( "Content-Type: text/javascript; charset=" . get_bloginfo( 'charset' ) );
App\Common\Protocols\Http\header( "Vary: Accept-Encoding" ); // Handle proxies
App\Common\Protocols\Http\header( "Expires: " . gmdate( "D, d M Y H:i:s", time() + $expiresOffset ) . " GMT" );
}
 ...
 *
 * This function complements theApp\Common\Protocols\Http\jump_exit() PHP function. The difference is that
...
function _default_wp_die_handler( $message, $title = '', $args = array() ) {
	...
	if ( ! did_action( 'admin_head' ) ) :
		if ( !headers_sent() ) {
			...
			nocache_headers();
		App\Common\Protocols\Http\header( 'Content-Type: text/html; charset=utf-8' );
		}
...
function _ajax_wp_die_handler( $message = '' ) {
	...
	App\Common\Protocols\Http\jump_exit( (string) $message );
App\Common\Protocols\Http\jump_exit( '0' );
}
...
function wp_send_json( $response ) {
	App\Common\Protocols\Http\header(( 'Content-Type: application/json; charset=' . get_option( 'blog_charset' ) ));
...
function send_nosniff_header() {
	App\Common\Protocols\Http\header( 'X-Content-Type-Options: nosniff' );
}
...
function send_frame_options_header() {
	App\Common\Protocols\Http\header( 'X-Frame-Options: SAMEORIGIN' );
}
