...
function wp_unregister_GLOBALS() {
	...
	App\Common\Protocols\Http\jump_exit( 'GLOBALS overwrite attempt detected' );
...
function wp_check_php_mysql_versions() {
	...
	App\Common\Protocols\Http\jump_exit( sprintf( __( 'Your server is running PHP version %1$s but WordPress %2$s requires at least %3$s.' ), $php_version, $wp_version, $required_php_version ) );
	}

	if ( ! extension_loaded( 'mysql' ) && ! file_exists( WP_CONTENT_DIR . '/db.php' ) ) {
		wp_load_translations_early();
	App\Common\Protocols\Http\jump_exit( __( 'Your PHP installation appears to be missing the MySQL extension which is required by WordPress.' ) );
	}
}
...
function wp_favicon_request() {
	...
	App\Common\Protocols\Http\header('Content-Type: image/vnd.microsoft.icon');
	App\Common\Protocols\Http\header('Content-Length: 0');
...
function wp_maintenance() {
	...
	if ( 'HTTP/1.1' != $protocol && 'HTTP/1.0' != $protocol )
		$protocol = 'HTTP/1.0';
App\Common\Protocols\Http\header( "$protocol 503 Service Unavailable", true, 503 );
App\Common\Protocols\Http\header( 'Content-Type: text/html; charset=utf-8' );
App\Common\Protocols\Http\header( 'Retry-After: 600' );
?>
	<!DOCTYPE html>
