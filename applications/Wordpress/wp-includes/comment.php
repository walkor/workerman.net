...
function wp_set_comment_cookies($comment, $user) {
	...
	App\Common\Protocols\Http\setcookie('comment_author_' . COOKIEHASH, $comment->comment_author, time() + $comment_cookie_lifetime, COOKIEPATH, COOKIE_DOMAIN);
	App\Common\Protocols\Http\setcookie('comment_author_email_' . COOKIEHASH, $comment->comment_author_email, time() + $comment_cookie_lifetime, COOKIEPATH, COOKIE_DOMAIN);
	App\Common\Protocols\Http\setcookie('comment_author_url_' . COOKIEHASH, esc_url($comment->comment_author_url), time() + $comment_cookie_lifetime, COOKIEPATH, COOKIE_DOMAIN);
}
...
function wp_allow_comment($commentdata) {
	...
	if ( $wpdb->get_var($dupe) ) {
		...
		App\Common\Protocols\Http\jump_exit( __('Duplicate comment detected; it looks as though you&#8217;ve already said that!') );
...
function check_comment_flood_db( $ip, $email, $date ) {
	...
	if ( $lasttime = $wpdb->get_var( $wpdb->prepare( "SELECT `comment_date_gmt` FROM `$wpdb->comments` WHERE `comment_date_gmt` >= %s AND ( `comment_author_IP` = %s OR `comment_author_email` = %s ) ORDER BY `comment_date_gmt` DESC LIMIT 1", $hour_ago, $ip, $email ) ) ) {
		...
		if ( $flood_die ) {
			...
			if ( defined('DOING_AJAX') )
			App\Common\Protocols\Http\jump_exit( __('You are posting comments too quickly. Slow down.') );
