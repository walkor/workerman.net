...
function wp_image_editor($post_id, $msg = false) {
	...
	App\Common\Protocols\Http\jump_exit( __('Image data does not exist. Please re-upload the image.') );
...
function wp_stream_image( $image, $mime_type, $post_id ) {
	if ( $image instanceof WP_Image_Editor ) {
		...
		switch ( $mime_type ) {
			case 'image/jpeg':
			App\Common\Protocols\Http\header( 'Content-Type: image/jpeg' );
				return imagejpeg( $image, null, 90 );
			case 'image/png':
			App\Common\Protocols\Http\header( 'Content-Type: image/png' );
				return imagepng( $image );
			case 'image/gif':
			App\Common\Protocols\Http\header( 'Content-Type: image/gif' );
