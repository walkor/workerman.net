# Summary

* [版权信息](license/README.md)
* [WorkerMan手册](manual/README.md)
   * [序言](manual/preface.md)
* [入门指引](start/README.md)
   * [简介](start/introduction.md)
   * [WorkerMan的特性](start/feature.md)
* [开发前必读](devread/README.md)
* [流程与规范](standards/README.md)
   * [目录结构](start/directory.md)
   * [系统流程](start/systemprocess.md)
   * [开发流程](start/developmentprocess.md)
   * [相关规范](start/namingconventions.md)
* [安装与配置](configure/README.md)
   * [环境要求](start/environment.md)
   * [安装](start/getworkerman.md)
   * [启动与停止](start/commend.md)
   * [主配置说明](configure/mainconfig.md)
   * [应用配置说明](configure/appconfig.md)
   * [FileMonitor.conf](configure/filemonitorconf.md)
   * [Monitor.conf](configure/monitorconf.md)
   * [Statistics统计模块](configure/statisticsconf.md)
* [基础模型开发流程](dev/README.md)
   * [制定协议](dev/protocols.md)
   * [实现dealInput/dealProcess](dev/dealinput.md)
   * [配置与启动](dev/configapp.md)
   * [基本流程开发示例](start/example.md)
* [Gateway/Worker开发流程](start/gatewaydev.md)
   * [基本流程](start/base.md)
   * [快速入门](start/interface.md)
   * [配置与启动](advanced/configrestart.md)
   * [Gateway/Worker开发示例](start/gateway_example.md)
   * [Config/Store 配置](start/store_config.md)
* [类函数等参考](funcref/README.md)
   * [SocketWorker类](funcref/socketworker.md)
       * [onStart](funcref/onstart.md)
       * [dealInput](funcref/socketworkerdealinput.md)
       * [dealProcess](funcref/socketworkerdealprocess.md)
       * [onStop](funcref/socketworkeronstop.md)
       * [onReload](funcref/socketworkeronreload.md)
       * [sendToClient](funcref/sendtoclient.md)
       * [getRemoteAddress](funcref/getremoteaddress.md)
       * [closeClient](funcref/closeclient.md)
   * [Gateway类](funcref/gateway.md)
       * [sendToAll](funcref/gatewaysendtoall.md)
       * [sendToClient](funcref/gatewaysendtouid.md)
       * [sendToCurrentClient](funcref/gatewaysendtocurrentuid.md)
       * [kickClient](funcref/gatewaykickuid.md)
       * [kickCurrentClient](funcref/gatewaykickcurrentuser.md)
       * [isOnline](funcref/gatewayisonline.md)
       * [getOnlineStatus](funcref/gatewaygetonlinestatus.md)
   * [Event类](funcref/eventphp.md)
       * [onGatewayConnect](funcref/eventongatewayconnect.md)
       * [onGatewayMessage](funcref/eventongatewaymessage.md)
       * [onMessage](funcref/eventonmessage.md)
       * [onClose](funcref/eventonclose.md)
   * [超全局数组$_SESSION](funcref/session.md)
       * [什么是超全局数组$_SESSION](funcref/what_is_session.md)
       * [使用场景及注意事项](funcref/use_session.md)
       * [超全局数组$_SESSION原理](funcref/how_to_work.md)
   * [超全局数组$_SERVER](funcref/server.md)
       * [什么是超全局数组$_SERVER](funcref/what_is_server.md)
       * [使用场景及注意事项](funcref/use_server.md)
       * [超全局数组$_SERVER原理](funcref/server_how_to_work.md)
* [调试](debug/README.md)
   * [基本调试](debug/base_debug.md)
   * [网络抓包](debug/tcpdump.md)
   * [跟踪系统调用](debug/strace.md)
* [安全](safety/README.md)
* [统计监控模块](statistics/README.md)
   * [关于统计监控系统](statistics/what_is_statistics.md)
   * [统计监控系统特点](statistics/statistics_feature.md)
   * [原理](statistics/principle.md)
   * [如何使用](statistics/how_to_use.md)
   * [权限验证](statistics/auth_check.md)
* [WorkerMan分布式部署扩容](distributed/README.md)
   * [WorkerMan分布式好处](distributed/why-distributed.md)
   * [如何分布式部署WorkerMan](distributed/how-distributed.md)
   * [gateway worker 分离部署](distributed/gateway-worker-separation.md)
* [常见问题](faq/README.md)
   * [WorkerMan下扩展的安装](faq/extension-install.md)
   * [WorkerMan日志](faq/debugging.md)
* [WorkerMan作为Web服务器](webserver/README.md)
   * [序言](webserver/webserver-preface.md)
   * [配置](webserver/webserver-config.md)
   * [与普通Web开发异同](webserver/webserver-differences.md)
* [高级应用](advanced/README.md)
   * [查看运行状态](start/workerman.md)
   * [telnet远程登录控制](advanced/telnet.md)
   * [心跳检测](advanced/heartbeat.md)
   * [定时任务](advanced/task.md)
* [压力测试](stresstest/README.md)
* [WorkerMan进程模型](process-model/README.md)
   * [master-worker模型](process-model/master-worker.md)
   * [master-gateway-worker模型](process-model/master-gateway-worker.md)
* [一些开发示例](demos/README.md)
   * [文件传输(二进制协议)](demos/file-transfer.md)
   * [文件传输(文本协议)](demos/file_transfer2.md)
   * [Mysql数据库的使用](demos/mysql-width-workerman.md)
   * [p2p之UDP打洞](demos/p2p-udp-hole-punching.md)
* [待续....](continue/README.md)

