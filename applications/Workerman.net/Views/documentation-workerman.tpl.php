...
Connection closed by foreign host.</code></pre>
    ...
    <p><b>[可选实现]</b> 当业务进程启动的时候出发一次，这里可以做一些全局的初始化的工作，例如初始化配置、数据库连接等。</p>
    <h4>钩子函数onStop</h4>
    ...
    <br>4、workerman检测到业务进程内存泄露达到上限值 conf/conf.d/Monitor.conf 中的max_mem_limit重启对应进程</p>
    <h4>钩子函数dealInput</h4>
    ...
    <p><b>[必须实现]</b>当数据全部接收完毕时（既dealInput()返回0时）触发。参数$bin是接收到的全部数据，根据选定协议解析出请求内容做相应的业务处理</p>
    <h4>void sendToClient($response_bin);</h4>
    <p>sendToClient方法用来将响应数据写到客户端。sendToClient是一个异步的过程，如果因客户端socket缓冲区满等原因无法发送全部数据，则workerman会异步等待，直到客户端可写则继续发送剩下的数据。</p>
	</div>
</div>